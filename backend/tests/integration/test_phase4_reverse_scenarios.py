...
async def execution_manager_unstable(test_db_session, unstable_client_factory, collection_manager):
    ...
    return QueryExecutionManager(
        test_db_session, unstable_client_factory, collection_manager, config
    )
...
class TestBoundaryConditions:
    ...
    async def test_empty_collection_operations(self, collection_manager):
        ...
        metrics = await collection_manager._calculate_collection_metrics(
        ...
        search_filter = QuerySearchFilter(text="nonexistent", limit=100)
    ...
    async def test_maximum_query_limits(self, collection_manager):
        ...
        for i in range(100):  # Use 100 instead of 1000 for practical testing
            large_dataset.append(
                {"id": str(uuid4()), "is_active": True, "complexity_score": float(i % 10)}
            )
    ...
    async def test_extremely_long_query_content(self, collection_manager):
        ...
        collection_schema = QueryCollectionCreate(
            ...
            initial_queries=[
                QueryCreate(
                    name="Long Query", query_text=long_content, variables={}, created_by="test-user"
                )
            ],
        )
...
class TestResourceExhaustion:
    ...
    async def test_storage_limit_exceeded(self, limited_storage_manager):
        ...
        try:
            storage_key = await limited_storage_manager.store_result(
                execution_id, query_id, large_data
            )
    ...
    async def test_concurrent_execution_overload(
        self, execution_manager_unstable, collection_manager
    ):
    ...
    async def test_memory_pressure_scenarios(self, limited_storage_manager):
        ...
        for i in range(1000):
            ...
            try:
                await limited_storage_manager.store_result(execution_id, query_id, small_data)
            except Exception:
...
class TestRaceConditions:
    ...
    async def test_concurrent_collection_modifications(self, collection_manager):
        ...
        async def add_query(index):
            query_schema = QueryCreate(
                name=f"Concurrent Query {index}", content=f"query {{ field{index} }}", variables={}
            )
    ...
    async def test_cache_consistency_under_concurrency(self, collection_manager):
        ...
        async def update_collection():
            ...
            update_schema = QueryCollectionUpdate(name=f"Updated {time.time()}")
...
class TestDataCorruption:
            ...
            try:
                ...
                try:
                    await collection_manager._add_query_to_collection(
                        MagicMock(), query_schema, validate=True
                    )
                    ...
                    assert any(
                        term in str(e).lower()
                        for term in ["invalid", "graphql", "corrupted", "query"]
                    )
                except Exception as e:
                ...
                assert (
                    "validation" in str(schema_error).lower()
                    or "graphql" in str(schema_error).lower()
                )
    ...
    async def test_invalid_metadata_recovery(self, limited_storage_manager):
        ...
        try:
            storage_key = await limited_storage_manager.store_result(
                execution_id, query_id, result_data
            )
...
class TestCacheInvalidation:
    ...
    async def test_cache_corruption_recovery(self, limited_storage_manager):
        ...
        limited_storage_manager._cache[storage_key] = ("corrupted_data", datetime.now(UTC))
...
class TestStateTransitions:
    ...
    async def test_invalid_query_status_transitions(self, collection_manager):
        ...
        result = await collection_manager.update_query(query_id, update_schema, validate=False)
...
class TestLongRunningOperations:
    ...
    async def test_long_running_batch_execution(
        self, execution_manager_unstable, collection_manager
    ):
        ...
        try:
            batch_result = await asyncio.wait_for(
                execution_manager_unstable.execute_batch(
                    query_ids, endpoint_id, mode=BatchMode.SEQUENTIAL
                ),
