...
def test_client():
    ...
    client.set_default_response(
        {"data": {"test": "result"}, "_complexity_score": 5.2, "_execution_time": 0.15}
    )
    return client
...
async def execution_manager(test_db_session, test_client_factory, query_collection_manager):
    ...
    return QueryExecutionManager(
        test_db_session, test_client_factory, query_collection_manager, config
    )
...
class TestQueryCollectionIntegration:
    ...
    async def test_create_collection_with_queries(self, query_collection_manager, sample_queries):
    ...
    async def test_search_queries_with_filters(self, query_collection_manager, sample_queries):
...
class TestExecutionManagerIntegration:
    ...
    async def test_single_query_execution(
        self, execution_manager, query_collection_manager, sample_endpoint, sample_queries
    ):
        ...
        with patch.object(query_collection_manager, "get_query", return_value=mock_query):
            ...
            result = await execution_manager.execute_query(query_id, sample_endpoint.pk_endpoint)
        ...
        for i, query_id in enumerate(query_ids):
            ...
            with patch.object(query_collection_manager, "get_query", return_value=mock_query):
                ...
                batch_result = await execution_manager.execute_batch(
                    query_ids, sample_endpoint.pk_endpoint, mode=BatchMode.PARALLEL
                )
    ...
    async def test_batch_execution_priority_mode(
        ...
        with patch.object(query_collection_manager, "get_query", side_effect=get_query_side_effect):
            ...
            batch_result = await execution_manager.execute_batch(
                query_ids, sample_endpoint.pk_endpoint, mode=BatchMode.PRIORITY
            )
    ...
    async def test_scheduled_execution(
        self, execution_manager, query_collection_manager, sample_endpoint
    ):
        ...
        scheduled = await execution_manager.schedule_query(
            query_id, sample_endpoint.pk_endpoint, cron_expression
        )
...
class TestResultStorageIntegration:
    ...
    async def test_store_and_retrieve_result(self, result_storage_manager):
        ...
        result_data = {
            ...
            "extensions": {"complexity": 5.2, "execution_time": 0.15},
        }

        # Store result
        storage_key = await result_storage_manager.store_result(
            execution_id, query_id, result_data, metadata={"test": "metadata"}
        )
    ...
    async def test_result_compression_and_serialization(self, result_storage_manager):
        """Test result compression and different serialization formats."""
        # Create large result data to test compression
        large_data = {
            "data": {"items": [{"id": i, "data": f"test data item {i}" * 100} for i in range(1000)]}
        }

        execution_id = uuid4()
        query_id = uuid4()

        storage_key = await result_storage_manager.store_result(execution_id, query_id, large_data)
    ...
    async def test_result_search_and_analytics(self, result_storage_manager):
        ...
        filter_params = ResultSearchFilter(
            query_ids=[uuid4()], min_size_bytes=500, max_size_bytes=2000, limit=10
        )
...
class TestEndToEndIntegration:
    ...
    async def test_complete_query_lifecycle(
        ...
        with patch.object(query_collection_manager, "get_query", return_value=query):
            ...
            execution_result = await execution_manager.execute_query(
                query.id, sample_endpoint.pk_endpoint
            )
            ...
            # Step 3: Store result
            storage_key = await result_storage_manager.store_result(
                execution_result.execution_id, query.id, execution_result.result_data
            )
        ...
        filter_params = ResultSearchFilter(query_ids=[query.id], limit=10)
    ...
    async def test_performance_monitoring_integration(
        self, execution_manager, result_storage_manager
    ):
...
class TestErrorHandlingIntegration:
    ...
    async def test_storage_failure_recovery(self, result_storage_manager):
        ...
        with pytest.raises(Exception):
            await result_storage_manager.store_result(execution_id, query_id, {"test": "data"})
