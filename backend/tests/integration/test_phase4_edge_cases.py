        ...
        for problematic_string in problematic_strings:
            try:
                query_schema = QueryCreate(
                    name=f"Control Char Test: {problematic_string[:20]!r}",
...
class TestTimezoneEdgeCases:
    ...
    async def test_timezone_transitions(self, collection_manager):
        ...
        timezone_test_cases = [
            datetime(2023, 3, 12, 2, 30, tzinfo=UTC),  # Spring forward
            datetime(2023, 11, 5, 1, 30, tzinfo=UTC),  # Fall back
            datetime(1970, 1, 1, 0, 0, tzinfo=UTC),  # Unix epoch
            datetime(2038, 1, 19, 3, 14, 7, tzinfo=UTC),  # Y2038 problem
            datetime(3000, 12, 31, 20, 0, 0, tzinfo=UTC),  # Far future
        ]
    ...
    async def test_timezone_aware_vs_naive_datetime(self, collection_manager):
        ...
        for dt in mixed_times:
            search_filter = QuerySearchFilter(created_after=dt, limit=1)
    ...
    async def test_complexity_score_precision(self, collection_manager):
        ...
        for score in precision_test_cases:
            collection_manager.complexity_analyzer.analyze_query.return_value = MagicMock(
                complexity_score=score, estimated_execution_time=0.1, field_count=1, depth=1
            )
...
class TestJSONSerializationEdgeCases:
    ...
    async def test_circular_reference_handling(self, limited_storage_manager):
        ...
        with pytest.raises(Exception) as exc_info:
            await limited_storage_manager.store_result(execution_id, query_id, data)
    ...
    async def test_non_serializable_objects(self, limited_storage_manager):
        ...
        for obj in non_serializable_objects:
            try:
                await limited_storage_manager.store_result(execution_id, query_id, obj)
                # If it succeeds, the object was converted to serializable form

            except Exception as e:
                # Expected for non-serializable objects
                error_msg = str(e).lower()
                assert any(term in error_msg for term in ["serializable", "json", "type", "object"])
        ...
        for depth in nesting_depths:
            ...
            try:
                storage_key = await limited_storage_manager.store_result(
                    execution_id, query_id, {"nested_data": data}
                )
                ...
                assert any(
                    term in error_msg
                    for term in ["recursion", "depth", "memory", "overflow", "limit"]
                )
...
class TestDatabaseConstraintViolations:
    ...
    async def test_null_constraint_violations(self, collection_manager):
        ...
        for data in problematic_data:
            try:
                ...
                assert any(
                    term in error_msg
                    for term in ["none", "null", "required", "string_type", "validation"]
                )
    ...
    async def test_foreign_key_violations(self, collection_manager):
        ...
        query_schema = QueryCreate(
            name="Orphan Query", query_text="query { test }", variables={}, created_by="test-user"
        )

        # Mock get_collection to return None
        collection_manager.get_collection = AsyncMock(return_value=None)

        result = await collection_manager.add_query(non_existent_collection_id, query_schema)
...
class TestFileSystemEdgeCases:
    ...
    async def test_filesystem_permission_errors(self, tmp_path):
        ...
        try:
            ...
            success = await backend.store("test_key", b"test_data", {"test": "metadata"})
    ...
    async def test_filesystem_space_exhaustion(self, tmp_path):
        ...
        try:
            success = await backend.store("large_key", large_data, {"size": len(large_data)})
            ...
            assert any(
                term in error_msg
                for term in ["space", "disk", "storage", "permission", "no such file"]
            )
        ...
        for malicious_key in malicious_keys:
            try:
                success = await backend.store(
                    malicious_key, b"malicious_data", {"attack": "path_traversal"}
                )
...
class TestNetworkProtocolEdgeCases:
    ...
    async def test_extremely_large_responses(self):
        ...
        large_response = {"data": {"items": [{"id": i, "data": "x" * 1000} for i in range(10000)]}}
    ...
    async def test_malformed_graphql_responses(self):
        ...
        for response in malformed_responses:
            ...
            try:
                ...
                assert any(
                    term in error_msg
                    for term in ["malformed", "invalid", "json", "response", "format", "attribute"]
                )
...
class TestMemoryAndResourceLimits:
    ...
    async def test_memory_exhaustion_simulation(self, limited_storage_manager):
        ...
        try:
            ...
            error_msg = str(e).lower()
            assert any(
                term in error_msg for term in ["memory", "size", "limit", "too large", "overflow"]
            )
            ...
            for storage_key in large_objects:
                try:
                    ...
                    logging.getLogger(__name__).debug(
                        f"Failed to cleanup storage key during test: {e}"
                    )
