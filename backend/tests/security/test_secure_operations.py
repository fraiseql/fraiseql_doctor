...
from fraiseql_doctor.services.retry import RetryConfig, RetryableClient
    ...
    async def test_pickle_warnings(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with warnings.catch_warnings(record=True) as w:
                ...
                serialized = await storage._serialize_data(test_data)
                assert len(w) > 0
                assert "unsafe" in str(w[0].message).lower() or "pickle" in str(w[0].message).lower()
    ...
    async def test_end_to_end_secure_storage(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with warnings.catch_warnings(record=True) as w:
                ...
                pickle_warnings = [warning for warning in w if "pickle" in str(warning.message).lower()]
