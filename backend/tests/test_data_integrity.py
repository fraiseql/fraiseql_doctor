...
async def test_query_name_uniqueness(db_session):
    ...
    # Create first query
    query1 = Query(name="test-query", query_text="query { user { id } }", created_by="test")
...
async def test_endpoint_url_validation(db_session):
    ...
    # Verify it was saved
    result = await db_session.execute(select(Endpoint).where(Endpoint.name == "test-endpoint"))
...
async def test_query_variables_jsonb_storage(db_session):
    ...
    complex_variables = {
        "filters": {"status": ["active", "pending"], "createdAfter": "2024-01-01T00:00:00Z"},
    ...
    result = await db_session.execute(select(Query).where(Query.name == "complex-query"))
...
async def test_endpoint_name_uniqueness(db_session):
    ...
    endpoint1 = Endpoint(
        name="api-endpoint", url="https://api1.example.com/graphql", auth_type="none"
    )
