...
def test_query_name_validation():
    ...
    query = QueryCreate(name="  valid-name  ", query_text="query { test }", created_by="test")
...
def test_query_text_validation():
    ...
    for query_text in valid_queries:
        query = QueryCreate(name="test-query", query_text=query_text, created_by="test")
...
def test_endpoint_url_validation():
    ...
    for url in valid_urls:
        endpoint = EndpointCreate(name="test-endpoint", url=url, auth_type="none")
    ...
    for url in invalid_urls:
        with pytest.raises(ValidationError):
            EndpointCreate(name="test-endpoint", url=url, auth_type="none")
...
def test_endpoint_auth_type_validation():
    ...
    for auth_type in valid_auth_types:
        endpoint = EndpointCreate(
            name="test-endpoint", url="https://api.example.com/graphql", auth_type=auth_type
        )
