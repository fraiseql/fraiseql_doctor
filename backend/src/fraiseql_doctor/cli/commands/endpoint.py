...
from ..utils.formatters import format_endpoint_detail, format_endpoint_table, format_progress_bar
...
def get_endpoint_manager():
    ...
    class MockQueryResult:
        ...
        def first(self):
            return MockEndpoint("Found Endpoint")
...
def add_endpoint(
    ...
    test_connection: bool = typer.Option(
        True, "--test/--no-test", help="Test connection after adding"
    ),
):
    ...
    try:
        ...
        if auth_method:
            ...
            if auth_method == "bearer":
                ...
                rprint(
                    f"[red]Error: Unknown auth method '{auth_method}'. Use: bearer, apikey, basic, none[/red]"
                )
                raise typer.Exit(1)
        ...
        if test_connection:
            ...
            try:
                ...
                if "errors" not in result:
                    ...
                    rprint(
                        f"[yellow]⚠[/yellow] Connection established but GraphQL errors: {result['errors']}"
                    )
    ...
    try:
        ...
        if format == "table":
            if show_urls:
                ...
                table = Table(
                    title="GraphQL Endpoints", show_header=True, header_style="bold magenta"
                )
                ...
                for endpoint in endpoints:
                    auth_method = (
                        endpoint.auth_config.get("method", "none")
                    ...
                    table.add_row(endpoint.name, endpoint.url, auth_text, timeout_text)
        ...
        if format == "pretty":
            ...
            if not show_sensitive and endpoint.auth_config:
                ...
                if "token" in auth_config:
                    auth_config["token"] = (
                        auth_config["token"][:8] + "..." if len(auth_config["token"]) > 8 else "***"
                    )
    ...
    try:
        ...
        if new_name:
            ...
            existing = (
                ...
                .filter(Endpoint.name == new_name, Endpoint.pk_endpoint != endpoint.pk_endpoint)
                .first()
            )
            if existing:
        ...
        with console.status("[bold green]Updating endpoint..."):
            db_session.commit()
...
def remove_endpoint(
    ...
    try:
        ...
        rprint("[yellow]About to delete endpoint:[/yellow]")
