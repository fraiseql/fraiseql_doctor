...
def format_query_detail(query: Query) -> Panel:
    ...
    if query.expected_complexity_score:
        metadata_lines.append(f"[cyan]Complexity Score:[/cyan] {query.expected_complexity_score:.2f}")
    ...
    if query.created_at:
        metadata_lines.append(f"[cyan]Created:[/cyan] {query.created_at.strftime('%Y-%m-%d %H:%M:%S')}")
    
    if query.updated_at and query.updated_at != query.created_at:
        metadata_lines.append(f"[cyan]Updated:[/cyan] {query.updated_at.strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    if query.query_metadata:
        interesting_metadata = {
            k: v for k, v in query.query_metadata.items()
...
def format_endpoint_detail(endpoint: Endpoint) -> Panel:
    ...
    if endpoint.auth_config:
        ...
        if auth_method == 'bearer' and endpoint.auth_config.get('token'):
            token_preview = endpoint.auth_config['token'][:10] + "..." if len(endpoint.auth_config['token']) > 10 else endpoint.auth_config['token']
...
def format_execution_result(execution: Execution) -> Panel:
    ...
    if execution.execution_start:
        info_lines.append(f"[cyan]Started:[/cyan] {execution.execution_start.strftime('%Y-%m-%d %H:%M:%S')}")
    
    if execution.execution_end:
        info_lines.append(f"[cyan]Completed:[/cyan] {execution.execution_end.strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    if execution.response_size_bytes:
        ...
        size_text = f"{size_mb:.2f}MB" if size_mb >= 1 else f"{execution.response_size_bytes / 1024:.1f}KB"
