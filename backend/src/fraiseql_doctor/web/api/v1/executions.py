...
async def get_query_history(
    ...
    if favorite is not None and favorite:
        ...
        conditions.append(
            Execution.query.has(
                QueryModel.query_metadata.op('->>')('favorite').cast(db.bind.dialect.BOOLEAN) == True
            )
        )
...
async def get_execution_by_id(
    ...
    query = select(Execution).options(
        selectinload(Execution.query),
        selectinload(Execution.endpoint)
...
async def toggle_execution_favorite(
    ...
    query = select(Execution).options(selectinload(Execution.query)).where(
        Execution.pk_execution == execution_id
    )
    result = await db.execute(query)
    execution = result.scalar_one_or_none()
...
async def clear_all_executions(
    endpoint_id: Optional[str] = Query(None, description="Clear only executions for specific endpoint"),
    db: AsyncSession = Depends(get_database_session)
) -> dict[str, str]:
    """
