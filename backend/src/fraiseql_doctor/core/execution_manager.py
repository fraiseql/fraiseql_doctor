...
from .query_collection import QueryCollectionManager, QueryPriority
...
class ExecutionConfig:
    ...
    priority_weights: dict[QueryPriority, int] = field(
...
class QueryExecutionManager:
        ...
        try:
            async with self._execution_semaphore:
                ...
                if "errors" in result_data:
                    error_messages = [
                        err.get("message", "Unknown error") for err in result_data["errors"]
                    ]
                    return ExecutionResult(
                        execution_id=execution_id,
                        query_id=query_id,
    ...
    async def execute_batch(
        ...
        logger.info(
            f"Starting batch execution {batch_id} with {len(query_ids)} queries in {mode.value} mode"
        )
        ...
        logger.info(
            f"Batch {batch_id} completed: {successful} successful, {failed} failed, {cancelled} cancelled in {total_time:.2f}s"
        )
    ...
    async def _execute_parallel(
        ...
        for query_id in query_ids:
            variables = variables_map.get(query_id)
            task = asyncio.create_task(self.execute_query(query_id, endpoint_id, variables, config))
            tasks.append(task)
    ...
    async def _execute_adaptive(
        ...
        simple_queries = [
            qid for qid, complexity in query_complexities if complexity <= simple_threshold
        ]
        complex_queries = [
            qid for qid, complexity in query_complexities if complexity > simple_threshold
        ]
        ...
        if simple_queries:
            ...
            simple_results = await self._execute_parallel(
                simple_queries, endpoint_id, variables_map, config
            )
        ...
        if complex_queries:
            ...
            complex_results = await self._execute_sequential(
                complex_queries, endpoint_id, variables_map, config
            )
    ...
    async def schedule_query(
        ...
        logger.info(
            f"Scheduled query {query_id} with expression '{cron_expression}', next run: {next_run}"
        )
    ...
    async def unschedule_query(self, scheduled_id: UUID) -> bool:
        ...
        if scheduled_id in self._scheduled_executions:
            ...
            await self.db_session.execute(
                "DELETE FROM scheduled_executions WHERE id = $1", [scheduled_id]
            )
            ...
            try:
                ...
                for scheduled in list(self._scheduled_executions.values()):
                    if (
                        ...
                        and now >= scheduled.next_execution
                    ):
                        # Execute query
    ...
    async def _execute_scheduled_query(self, scheduled: ScheduledExecution):
        ...
        try:
            result = await self.execute_query(
                scheduled.query_id, scheduled.endpoint_id, config_override=scheduled.config
            )
    ...
    async def _store_execution_result(self, result: ExecutionResult):
        ...
        execution = Execution(
            ...
            execution_context={"cache_hit": result.cache_hit}
    ...
    def _serialize_config(self, config: ExecutionConfig) -> dict:
        ...
        if "priority_weights" in result:
            result["priority_weights"] = {str(k): v for k, v in result["priority_weights"].items()}
        return result
    ...
    async def get_query_execution_history(
        self, query_id: UUID, limit: int = 100
    ) -> list[Execution]:
    ...
    async def start(self):
        ...
        logger.info(
            f"Execution manager started with {len(self._scheduled_executions)} scheduled executions"
        )
