...
class DatabaseStorageBackend:
    ...
    async def store(self, key: str, data: bytes, metadata: dict[str, Any]) -> bool:
        ...
        try:
            ...
            """,
                [key, data, json.dumps(metadata), datetime.now(UTC)],
            )
    ...
    async def retrieve(self, key: str) -> bytes | None:
        ...
        try:
            result = await self.db_session.execute(
                "SELECT data FROM result_storage WHERE key = $1", [key]
            )
    ...
    async def delete(self, key: str) -> bool:
        ...
        try:
            result = await self.db_session.execute(
                "DELETE FROM result_storage WHERE key = $1", [key]
            )
            await self.db_session.commit()
    ...
    async def exists(self, key: str) -> bool:
        ...
        try:
            result = await self.db_session.execute(
                "SELECT 1 FROM result_storage WHERE key = $1", [key]
            )
            return len(result) > 0
    ...
    async def list_keys(self, prefix: str = "") -> list[str]:
        ...
        try:
            if prefix:
                result = await self.db_session.execute(
                    "SELECT key FROM result_storage WHERE key LIKE $1", [f"{prefix}%"]
                )
            else:
                result = await self.db_session.execute("SELECT key FROM result_storage")
...
class ResultStorageManager:
    ...
    def __init__(self, db_session, config: StorageConfig = None):
        self.db_session = db_session
    ...
    async def store_result(
        ...
        try:
            ...
            # Cache small results
            if (
                self.config.cache_small_results
                and compressed_size < self.config.cache_threshold_kb * 1024
            ):
            ...
            logger.info(
                f"Stored result {storage_key}, compressed {original_size} -> {compressed_size} bytes"
            )
    ...
    async def retrieve_result(
        ...
        try:
            ...
            if storage_key in self._cache:
                ...
                # Check TTL
                if (datetime.now(UTC) - cached_at).total_seconds() < self.config.ttl_hours * 3600:
                    self._cache_hits += 1
            ...
            if (
                self.config.cache_small_results
                and len(compressed_data) < self.config.cache_threshold_kb * 1024
            ):
                self._cache[storage_key] = (compressed_data, datetime.now(UTC))
    ...
    async def delete_result(self, storage_key: str) -> bool:
        ...
        try:
            ...
            await self.db_session.execute(
                "DELETE FROM query_results WHERE storage_key = $1", [storage_key]
            )
            await self.db_session.commit()
    ...
    async def search_results(self, filter_params: ResultSearchFilter) -> list[QueryResult]:
    ...
    async def get_storage_analytics(self, days_back: int = 7) -> dict[str, Any]:
    ...
    async def _store_result_record(
        self, execution_id: UUID, query_id: UUID, storage_key: str, metadata: dict[str, Any]
    ):
        """Store result record in database."""
    ...
    def _update_storage_metrics(
        self, original_size: int, compressed_size: int, start_time: datetime
    ):
        ...
        # Update averages
        self._metrics.avg_size_bytes = self._metrics.total_size_bytes / self._metrics.total_results

        if original_size > 0:
            compression_ratio = compressed_size / original_size
            # Moving average for compression ratio
            self._metrics.compression_ratio = (
                self._metrics.compression_ratio * (self._metrics.total_results - 1)
                + compression_ratio
            ) / self._metrics.total_results

        # Storage time
        storage_time_ms = (datetime.now(UTC) - start_time).total_seconds() * 1000
    ...
    async def cleanup_expired_results(self) -> int:
        ...
        cutoff_time = datetime.now(UTC) - timedelta(hours=self.config.ttl_hours)
        ...
        logger.info(
            f"Cleaned up {deleted_count} expired results and {len(expired_cache_keys)} cache entries"
        )
