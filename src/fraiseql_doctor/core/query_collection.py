...
from ..models.query_collection import QueryCollection, QueryMetadata
from ..schemas.query import QueryCreate, QueryUpdate, QueryCollectionCreate, QueryCollectionUpdate
...
class QueryCollectionManager:
    ...
    async def list_collections(
        ...
        if include_metrics:
            for collection in collections:
                collection.metrics = await self._calculate_collection_metrics(collection.pk_query_collection)
    ...
    async def _add_query_to_collection(
        ...
        query = Query(
            ...
            query_metadata={
                ...
                "last_validated": datetime.now(timezone.utc).isoformat() if validate else None
            }
        )
        ...
        if schema.query_text is not None and schema.query_text != query.query_text:
            if validate:
                try:
                    ...
                    query.metadata.last_validated = datetime.now(timezone.utc)
                except Exception as e:
                    raise ValueError(f"Invalid GraphQL query: {e}")
    ...
    async def bulk_update_query_status(
        self,
        query_ids: List[UUID],
        status: QueryStatus
    ) -> int:
        ...
        result = await self.db_session.execute(
            ...
            [status.value, datetime.now(timezone.utc), query_ids]
        )
        ...
        for query_id in query_ids:
            if query_id in self._query_cache:
                ...
                self._query_cache[query_id].updated_at = datetime.now(timezone.utc)
