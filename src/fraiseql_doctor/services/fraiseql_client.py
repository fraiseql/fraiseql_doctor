...
class GraphQLClientError(Exception):
    ...
    def __init__(self, message: str, status_code: Optional[int] = None, response_time_ms: Optional[int] = None):
...
class FraiseQLClient:
    ...
    async def execute_query(
        ...
        try:
            ...
            response_time_ms = int((time.time() - start_time) * 1000)
            if isinstance(e, (GraphQLClientError, NetworkError, AuthenticationError, GraphQLExecutionError)):
                raise
            else:
                raise GraphQLClientError(
