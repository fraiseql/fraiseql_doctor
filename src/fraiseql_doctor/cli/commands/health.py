...
class MockQueryResult:
    ...
    def all(self): return [MockEndpoint("Health Test")]
    def first(self): return MockEndpoint("Health Test")
...
class GraphQLExecutionError(Exception): pass
...
def health_check(
    ...
    try:
        ...
        with format_progress_bar() as progress:
            task = progress.add_task(f"Checking {len(endpoints)} endpoints...", total=len(endpoints))
...
def monitor_health(
    endpoints: Optional[List[str]] = typer.Option(None, "--endpoint", "-e", help="Endpoint names to monitor"),
    ...
    try:
        ...
        rprint(f"[cyan]Monitoring {len(target_endpoints)} endpoints every {interval} seconds[/cyan]")
        if duration:
                    ...
                    for endpoint_name, current_data in current_results.items():
                        if endpoint_name in last_status:
                            ...
                            if old_status != new_status:
                                ...
                                rprint(f"[{status_color}]🔔 ALERT: {endpoint_name} changed from {old_status.upper()} to {new_status.upper()}[/{status_color}]")
...
def health_report(
    ...
    since: Optional[str] = typer.Option(None, "--since", help="Include results since (e.g., '1 hour ago', '2024-01-01')"),
    ...
    try:
        ...
        if format == "table":
            # Summary table
            summary_table = Table(
                title="Health Check Report Summary",
                show_header=True,
            ...
            for endpoint_name, stats in endpoint_stats.items():
                uptime_color = "green" if stats["uptime_percentage"] >= 95 else "yellow" if stats["uptime_percentage"] >= 90 else "red"
                ...
                summary_table.add_row(
                    ...
                    uptime_text,
                    f"{stats['avg_response_time']:.0f}ms" if stats["avg_response_time"] > 0 else "-",
...
"""
            ...
            for endpoint_name, stats in endpoint_stats.items():
                uptime_class = "uptime-good" if stats["uptime_percentage"] >= 95 else "uptime-warning" if stats["uptime_percentage"] >= 90 else "uptime-critical"
...
def health_status(
    ...
    try:
        ...
        for ep in endpoints:
            ...
            latest_check = db_session.query(HealthCheck).filter(
                HealthCheck.fk_endpoint == str(ep.pk_endpoint)
            ).order_by(HealthCheck.check_timestamp.desc()).first()
        ...
        if summary:
            ...
            unhealthy = len([s for s in current_status.values() if s["status"] in ["unhealthy", "timeout"]])
            unknown = total - healthy - unhealthy
            
            rprint(f"[cyan]Health Status Summary:[/cyan]")
