...
import yaml
try:
    from graphql import build_client_schema, get_introspection_query, validate, parse, GraphQLError
except ImportError:
            ...
            for definition in parsed.definitions:
                if hasattr(definition, 'operation'):
                    operations.append({
                        ...
                        'selection_count': len(definition.selection_set.selections) if definition.selection_set else 0
                    })
            ...
            for definition in parsed.definitions:
                if hasattr(definition, 'variable_definitions') and definition.variable_definitions:
                    for var_def in definition.variable_definitions:
                        ...
                        is_required = (
                            hasattr(var_def.type, 'kind') and 
                            var_def.type.kind == 'non_null_type' and
                            var_def.default_value is None
                        )
...
class ExportHandler:
    ...
    @staticmethod
    def export_queries(queries: List[Dict[str, Any]], output_path: Path, format: str = "json") -> None:
        """
